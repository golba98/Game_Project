...
function draw() {
    ...
    if (seasonTime >= SEASON_DURATION) {
        ...
        // Inline cycleSeason(1, false)
        currentSeasonIndex = (currentSeasonIndex + 1 + seasons.length) % seasons.length;
        // resetTimer is false, so don't reset seasonTime here (it was already adjusted)
    ...
    // Inline drawSky()
    {
        ...
        let sunMoonSize = 80;
        let starCount = 0;
        // We will store the overlay drawing task as an object or variables
    ...
    // Inline drawMountains()
    for (var i = 0; i < mountains.length; i++) {
        var m = mountains[i];
        // Inline drawMountain(m)
        let x = m.x;
        let y = floorPos_y;
        let w = m.width;
        let h = m.height;
    ...
    for (let i = 0; i < clouds.length; i++) {
        let c = clouds[i];
        // Inline drawCloud(c.x, c.y, c.color)
        let x = c.x;
        let y = c.y;
        let col = c.color;
        noStroke();
    ...
    for (var i = 0; i < trees.length; i++) {
        ...
        switch (currentSeason.name) {
            ...
            case "Summer":
                ...
                break;
            case "Autumn":
                // Inline drawAutumnTree
                {
                    let tree = t;
                    let season = currentSeason;
                    let baseColor = lerpColor(tree.leafColor, color(190, 110, 60), 0.7);
                    let highlight = lerpColor(baseColor, color(255, 160, 80), 0.5);
                    // Inline drawSeasonalCanopy(tree, baseColor, highlight) (sizeScale default 0.8)
                    let cx = tree.x + tree.trunkW / 2;
                    let baseY = tree.y;
                    let canopy = tree.canopySize * 0.8;
                    ...
                    noStroke();
                    for (let k = 0; k < 3; k++) {
                        let drop = (frameCount * 0.4 + tree.particlePhase * 30 + k * 25) % 60;
                        let xOffset = cos(frameCount * 0.02 + tree.particlePhase + k) * 25;
    ...
    // Inline checkCollectable()
    if (collectable.isFound == false) {
        // Inline drawCoin(collectable.x_pos, collectable.y_pos)
        push();
    ...
    // Inline isNearCaveEntrance check
    ...
    if (currentSeason.name === "Winter" && !isHibernating && isNearCave) {
        // Inline drawThoughtBubble(gameChar_x, gameChar_y - 120, "i need to sleep")
        let x = gameChar_x;
    ...
    if (gameChar_y > ORIGINAL_HEIGHT) {
        if (keyCode == 32) {
            ...
            for (var i = 0; i < 20; i++) {
                ...
                if (validPosition) {
                    let seasonStyle = currentSeason.leafStyle;
                    let baseLeaf = color(random(20, 60), random(100, 180), random(20, 60));
                    trees.push({
                        x: tx,
                        y: floorPos_y,
